...
@section('title', 'Portugal Certificate - ' . $application->application_reference)

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('admin.portugal-certificates.index') }}" class="inline-flex items-center text-emerald-600 hover:text-emerald-900 font-medium mb-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-start justify-between">
                <div class="flex items-center gap-4">
                    <div class="h-16 w-16 rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-white font-bold text-2xl shadow-lg">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        @if($application->portugal_nif)
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wide">Portugal NIF</p>
                            <p class="text-sm font-medium text-gray-900 font-mono">{{ $application->portugal_nif }}</p>
                        </div>
                        @endif
                        @if($application->portugal_residence_permit)
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wide">Residence Permit</p>
                            <p class="text-sm font-medium text-gray-900 font-mono">{{ $application->portugal_residence_permit }}</p>
                        </div>
                        @endif
                        @if($application->portugal_social_security_number)
                        <div>
                            <p class="text-xs text-gray-500 uppercase tracking-wide">Social Security Number</p>
                            <p class="text-sm font-medium text-gray-900 font-mono">{{ $application->portugal_social_security_number }}</p>
                        </div>
                        @endif
                    </div>
                </div>
            ...
            <!-- Portugal Residence History -->
            @if($application->portugal_residence_history && count($application->portugal_residence_history) > 0)
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    <div class="space-y-3">
                        @foreach($application->portugal_residence_history as $residence)
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div class="bg-gradient-to-r from-purple-50 to-pink-50 px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    ...
                    <a href="{{ route('admin.portugal-certificate.download-documents', $application) }}"
                            ...
                            @foreach($application->documents as $document)
                                <div class="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-purple-300 transition">
                                    ...
                                    <div class="flex gap-2">
                                        <a href="{{ route('admin.portugal-certificate.preview-document', $document) }}" target="_blank"
                                        ...
                                        <a href="{{ route('admin.portugal-certificate.download-document', $document) }}"
        ...
        <div class="lg:col-span-1">
            <div class="sticky top-4 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-lg border-2 border-emerald-300 overflow-hidden">
                    ...
                    <form action="{{ route('admin.portugal-certificates.update-status', $application) }}" method="POST" class="p-6 space-y-4">
                        @csrf
                        @method('PATCH')

                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Update Status</label>
                            <select name="status" id="status" class="mt-1 block w-full pl-3 pr-10 py-2.5 text-base border-gray-300 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 rounded-lg">
                        ...
                        <div>
                            ...
                            <textarea name="admin_notes" id="admin_notes" rows="4" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm" placeholder="Add internal note..."></textarea>
                        </div>

                        <button type="submit" class="w-full inline-flex items-center justify-center px-6 py-3 bg-emerald-600 border border-transparent rounded-lg font-semibold text-sm text-white hover:bg-emerald-700 shadow-lg transition-colors">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <div class="p-4">
                        @if($paymentPdfDoc)
                            <div class="flex gap-2">
                                <a href="{{ route('admin.portugal-certificate.preview-document', $paymentPdfDoc) }}" target="_blank"
                                ...
                                <a href="{{ route('admin.portugal-certificate.download-document', $paymentPdfDoc) }}"
