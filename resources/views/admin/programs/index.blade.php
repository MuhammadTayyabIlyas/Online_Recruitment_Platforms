        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $stats['with_applications'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $stats['by_degree'] }}</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $stats['by_country'] }}</p>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="GET" action="{{ route('admin.programs.index') }}" class="space-y-4">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.programs.index') }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium">
                    Clear Filters
                </a>
            </div>
        </form>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="p-6 border-b border-gray-100">
            <div class="flex items-center justify-between">
                ...
                <form method="POST" action="{{ route('admin.programs.bulk-action') }}" id="bulk-action-form">
                    @csrf
                    <div class="flex items-center gap-3">
                        <select
                            name="action"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($programs as $program)
                            <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <div class="flex items-center gap-2 mt-1">
                                            ...
                                            @if($program->tuition_fee)
                                                <span class="text-xs text-gray-600">
                                                    €{{ number_format($program->tuition_fee) }}/year
                                                </span>
                                            @endif
                                        </div>
                                    </div>
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-900">{{ $program->university->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-700">{{ $program->degree->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-700">{{ $program->country->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('admin.programs.show', $program) }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">
                                        {{ $program->applications_count }} applications
                                    </a>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-gray-700">{{ $program->creator->name ?? 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4">
        ...
        @else
            <div class="p-12 text-center">
                ...
                <p class="mt-2 text-sm text-gray-600">
                    @if(request()->hasAny(['search', 'degree', 'subject', 'country', 'study_mode']))
