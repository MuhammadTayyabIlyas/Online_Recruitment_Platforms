...
@section('title', 'Police Certificate Applications')
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Police Certificate Applications</h1>
            <p class="text-sm text-gray-500 mt-1">Manage UK Police Certificate applications</p>
        </div>
        <a href="{{ route('admin.police-certificates.export', request()->query()) }}"
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <form action="{{ route('admin.police-certificates.index') }}" method="GET">
            <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                ...
                <div>
                    ...
                    <select name="service_type" id="service_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        <option value="">All Types</option>
                        <option value="normal" {{ request('service_type') == 'normal' ? 'selected' : '' }}>Normal (14 days)</option>
                        <option value="urgent" {{ request('service_type') == 'urgent' ? 'selected' : '' }}>Urgent (7 days)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Name, Email, Reference" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    @if(request()->hasAny(['status', 'service_type', 'search', 'date_from', 'date_to']))
                        <a href="{{ route('admin.police-certificates.index') }}" class="bg-gray-100 border border-gray-300 rounded-md shadow-sm py-2 px-4 text-sm font-medium text-gray-700 hover:bg-gray-200">
                            Clear
                        </a>
                    @endif
                </div>
                ...
                @forelse($applications as $application)
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="text-sm font-mono font-medium text-indigo-600">{{ $application->application_reference }}</span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                            {{ $application->payment_amount_display }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $application->submitted_at?->format('M d, Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.police-certificates.show', $application) }}" class="text-indigo-600 hover:text-indigo-900">
                                View Details
                            </a>
                        </td>
                    </tr>
