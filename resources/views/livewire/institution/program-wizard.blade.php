<div class="max-w-4xl mx-auto py-8 px-4">
    ...
    <div class="mb-4 p-4 bg-gray-900 text-white rounded text-xs font-mono space-y-2">
        ...
        <div>Step: {{ $currentStep }}/{{ $totalSteps }} | Title: "{{ $title }}"</div>
            ...
            @for ($step = 1; $step <= $totalSteps; $step++)
                <div class="flex flex-col items-center flex-1 {{ $step < $totalSteps ? 'relative' : '' }}">
                    ...
                    <button
                        ...
                        {{ $currentStep > $step ? 'bg-green-500 text-white' : ($currentStep == $step ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600') }}
                    ...
                    <!-- Step Label -->
                    <span class="mt-2 text-xs font-medium {{ $currentStep == $step ? 'text-blue-600' : 'text-gray-500' }}">
                        @if($step == 1) Basic Info
                        @elseif($step == 2) Academic
                        @else Fees & Details
                        @endif
                    </span>
        ...
        <form wire:submit.prevent="submit">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-xs text-gray-500">
                                Can't find yours?
                                <a href="{{ route('institution.universities.create') }}" target="_blank" class="text-blue-600 hover:text-blue-800 font-medium">
                                    Add new institution
                                </a>
                            </p>
                        </div>
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                @foreach(['On-campus', 'Online', 'Hybrid'] as $mode)
                                    <label class="relative flex items-center justify-center p-3 rounded-lg border-2 cursor-pointer transition-all
                                        {{ $study_mode === $mode ? 'border-blue-500 bg-blue-50' : 'border-gray-300 hover:border-gray-400' }}">
                                        <input
                                            type="radio"
                                            ...
                                            class="sr-only"
                                        >
                                        <span class="text-sm font-medium {{ $study_mode === $mode ? 'text-blue-700' : 'text-gray-700' }}">
                                            {{ $mode }}
                                        </span>
                                    </label>
                                @endforeach
            ...
            <div class="flex items-center justify-between mt-8 pt-6 border-t border-gray-200">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('institution.programs.index') }}" class="px-4 py-2 text-gray-600 hover:text-gray-800 text-sm font-medium">
                        Cancel
                    </a>
