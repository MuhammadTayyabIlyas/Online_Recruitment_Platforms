...
@section('title', 'Move to Europe | Visas & Residency')
@section('meta_description', 'Compare European visa and residency options: Golden Visa, Passive Income, Digital Nomad, Startup, and Highly Skilled paths. Get a clear path, costs, and expert support.')
    ...
    <section id="pathways" class="bg-gray-50">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            <div class="flex flex-col md:flex-row md:items-end md:justify-between mb-8 gap-4">
                ...
                <a href="{{ route('contact') }}" class="inline-flex items-center gap-2 text-blue-700 font-semibold hover:text-blue-900">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                @foreach($pathways as $path)
                    <div class="p-6 bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3">
                        <div class="flex items-center justify-between">
                            <div class="text-2xl">{{ $path['flag'] }}</div>
                            <span class="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{{ $path['time'] }}</span>
                        </div>
                        <h3 class="text-lg font-semibold text-gray-900">{{ $path['title'] }}</h3>
                        <p class="text-sm text-gray-700">{{ $path['who'] }}</p>
                        <div class="text-sm text-gray-900 font-medium">Typical cost: {{ $path['cost'] }}</div>
                        <p class="text-sm text-gray-600">Best fit: {{ $path['countries'] }}</p>
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="grid md:grid-cols-2 gap-8 items-center">
                ...
                <div class="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                    ...
                    <div class="mt-6 flex flex-col sm:flex-row gap-3">
                        <a href="{{ route('contact') }}" class="inline-flex items-center justify-center gap-2 bg-white text-blue-700 font-semibold px-6 py-3 rounded-lg hover:bg-blue-50 transition">
                            Speak with an expert
                        </a>
                        <a href="{{ route('register') }}" class="inline-flex items-center justify-center gap-2 border border-white text-white font-semibold px-6 py-3 rounded-lg hover:bg-white/10 transition">
                            Create free account
                        </a>
                    </div>
                </div>
