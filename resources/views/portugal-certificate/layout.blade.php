...
$stepDescriptions = [
    ...
    6 => 'Sign the authorization letter',
        ...
        <div class="mb-8">
            ...
            <div class="md:hidden mb-4">
                <div class="flex justify-between items-center mb-2">
                    ...
                    <span class="text-sm font-bold text-emerald-600">{{ $percentage }}% Complete</span>
                </div>
                <div class="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                    <div class="h-full rounded-full transition-all duration-500 bg-gradient-to-r from-green-500 to-emerald-600"
                    ...
                    @for($i = 1; $i <= $totalSteps; $i++)
                        <div class="flex items-center">
                            <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300
                                {{ $i < $step ? 'bg-green-500 text-white' : ($i == $step ? 'bg-emerald-600 text-white ring-4 ring-emerald-200' : 'bg-gray-200 text-gray-500') }}">
                                @if($i < $step)
                            ...
                            @if($i < $totalSteps)
                                <div class="w-12 h-1 transition-all duration-300 {{ $i < $step ? 'bg-green-500' : 'bg-gray-200' }}"></div>
                            @endif
                        </div>
                    @endfor
                </div>
            ...
            <form action="{{ route('portugal-certificate.process-step', ['step' => $step]) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mt-8 pt-6 border-t border-gray-200">
                    ...
                    <div class="w-full sm:w-auto flex flex-col sm:flex-row gap-3 order-1 sm:order-2">
                        ...
                        <!-- Next/Submit Button -->
                        <button type="submit"
                                class="w-full sm:w-auto inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-semibold rounded-lg hover:from-green-700 hover:to-emerald-700 transition shadow-lg min-h-[48px]">
                            @if($step === 7)
                                Submit Application
