...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="text-sm text-gray-600">
            <span class="font-semibold">{{ $applications->total() }}</span> total applications
        </div>
    </div>

    <!-- Filters -->
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
        <form action="{{ route('employer.applicants.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
            ...
            <div class="flex items-end gap-2">
                ...
                @if(request()->hasAny(['job_id', 'status', 'search']))
                    <a href="{{ route('employer.applicants.index') }}" class="bg-gray-100 border border-gray-300 rounded-md shadow-sm py-2 px-4 text-sm font-medium text-gray-700 hover:bg-gray-200">
                        Clear
                    </a>
                @endif
            </div>
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-3 mb-2">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-sm text-gray-600 flex items-center">
                                            ...
                                            Applied for: <span class="font-medium ml-1">{{ $application->job->title }}</span>
                                        </p>
                                    </div>
                                ...
                                <div class="sm:hidden mt-3">
                                    <form action="{{ route('employer.applicants.status', $application) }}" method="POST" class="flex gap-2">
                                        @csrf
                                        @method('PATCH')
                            ...
                            <div class="ml-4 flex-shrink-0 flex flex-col gap-2">
                                ...
                                <div class="relative hidden sm:block">
                                    <form action="{{ route('employer.applicants.status', $application) }}" method="POST" class="flex gap-1">
                                        @csrf
                                        @method('PATCH')
                                        <select name="status"
