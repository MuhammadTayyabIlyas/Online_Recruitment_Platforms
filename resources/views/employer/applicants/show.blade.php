...
@section('title', 'Applicant Details - ' . $application->user->name)

@section('content')
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <!-- Header -->
    <div class="mb-6">
        <a href="{{ route('employer.applicants.index') }}" class="inline-flex items-center text-indigo-600 hover:text-indigo-900 font-medium mb-4">
            ...
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">Applied {{ $application->created_at->format('F j, Y, g:i a') }} ({{ $application->created_at->diffForHumans() }})</p>
                    </div>
                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-medium text-gray-900">{{ $application->user->email ?? 'Not provided' }}</p>
                        </div>
                    </div>
                        ...
                        <div class="flex items-start">
                            ...
                            <div>
                                ...
                                <p class="text-sm font-medium text-gray-900">
                                    @if($application->user->country_code)
                                        <span class="text-gray-600">({{ strtoupper($application->user->country_code) }})</span>
                                    @endif
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    @if($application->resume_path)
                        <a href="{{ route('employer.applicants.resume', $application) }}"
                        ...
                        <h2 class="text-lg font-bold text-gray-900 flex items-center">
                            ...
                            @if($application->user?->documents?->count() > 0)
                                <span class="ml-2 px-2 py-0.5 bg-indigo-100 text-indigo-800 text-xs font-semibold rounded-full">
                                    {{ $application->user->documents->count() }}
                                </span>
                            @endif
                        </h2>
                    </div>
                                        ...
                                        <div class="flex items-center flex-1 min-w-0">
                                            ...
                                            <div class="ml-4 flex-1 min-w-0">
                                                ...
                                                <p class="text-xs text-gray-500 mt-1">
                                                    {{ $document->file_size_formatted }} • Uploaded {{ $document->created_at->format('M d, Y') }}
                                                </p>
                                            </div>
                                        </div>
                                        <div class="ml-4">
        ...
        <div class="lg:col-span-1 space-y-6">
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden sticky top-6">
                ...
                <form action="{{ route('employer.applicants.status', $application) }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    @method('PATCH')
                    ...
                    <fieldset class="space-y-3">
                        ...
                        <label class="flex items-center p-3 rounded-lg border border-gray-200 {{ $application->user->phone ? 'hover:bg-gray-50 cursor-pointer' : 'bg-gray-50 opacity-60 cursor-not-allowed' }} transition">
                            <input type="checkbox" name="channels[]" value="whatsapp" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ $application->user->phone ? 'checked' : 'disabled' }}>
                            <span class="ml-3 flex items-center justify-between flex-1">
                                <span class="flex items-center text-sm font-medium">
                        ...
                        <label class="flex items-center p-3 rounded-lg border border-gray-200 {{ $application->user->phone ? 'hover:bg-gray-50 cursor-pointer' : 'bg-gray-50 opacity-60 cursor-not-allowed' }} transition">
                            <input type="checkbox" name="channels[]" value="viber" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ $application->user->phone ? 'checked' : 'disabled' }}>
                            <span class="ml-3 flex items-center justify-between flex-1">
                                <span class="flex items-center text-sm font-medium">
                    ...
                    @if($application->user->phone)
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800">
                            ...
                            <p class="mt-1">{{ $application->user->email }}</p>
                ...
                @if(session('contact_links'))
                    <div class="px-6 pb-6">
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm text-blue-800">
                            ...
                            <div class="space-y-1">
                                @foreach(session('contact_links') as $label => $url)
                                    <a href="{{ $url }}" target="_blank" rel="noopener" class="block underline text-blue-700 hover:text-blue-900 capitalize">
                                        {{ $label }}
                                    </a>
                                @endforeach
                            </div>
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <form action="{{ route('employer.applicants.notes.store', $application) }}" method="POST" class="p-6 space-y-4">
                    @csrf
                    <div>
                        <label for="notes" class="block text-sm font-medium text-gray-700 mb-2">Add your notes about this candidate</label>
                        <textarea name="notes" id="notes" rows="6" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-lg" placeholder="e.g., Strong technical skills, good communication, follow up in 2 days...">{{ $application->notes }}</textarea>
