                ...
                @for($i = 1; $i <= 7; $i++)
                    <div class="flex items-center">
                        <div class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm
                            {{ $i < $step ? 'bg-amber-500 text-white' : ($i == $step ? 'bg-amber-600 text-white' : 'bg-gray-200 text-gray-500') }}">
                            @if($i < $step)
                        ...
                        @if($i < 7)
                            <div class="w-8 h-1 {{ $i < $step ? 'bg-amber-500' : 'bg-gray-200' }}"></div>
                        @endif
                    </div>
                @endfor
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
            ...
            <form action="{{ route('greece-certificate.process-step', ['step' => $step]) }}" method="POST" enctype="multipart/form-data" class="p-6">
                ...
                <div class="flex justify-between mt-8 pt-6 border-t border-gray-200">
                    ...
                    <button type="submit"
                            class="inline-flex items-center px-6 py-3 bg-gradient-to-r from-amber-500 to-yellow-600 text-white font-semibold rounded-lg hover:from-amber-600 hover:to-yellow-700 transition shadow-lg">
                        @if($step === 7)
                            Submit Application
