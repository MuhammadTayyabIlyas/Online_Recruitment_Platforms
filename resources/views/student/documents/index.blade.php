        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2" id="totalCount">{{ $documents->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ $documents->where('is_verified', true)->count() }}</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold text-gray-900 mt-2">{{ 15 - $documents->count() }}</p>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div id="documentsList">
            @forelse($documents as $document)
                <div class="document-item px-6 py-4 border-b border-gray-200 hover:bg-gray-50 transition-colors" data-id="{{ $document->id }}">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center flex-1">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex items-center space-x-4 mt-1">
                                    ...
                                    <span class="text-sm text-gray-500">{{ $document->file_size_formatted }}</span>
                                    <span class="text-sm text-gray-400">{{ $document->created_at->diffForHumans() }}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center space-x-3 ml-4">
                            ...
                            @endif
                            <a href="{{ route('student.documents.download', $document) }}" class="text-indigo-600 hover:text-indigo-800 p-2">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/>
                                </svg>
                            </a>
                            <button onclick="deleteDocument({{ $document->id }})" class="text-red-600 hover:text-red-800 p-2">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    uploadForm.addEventListener('submit', async (e) => {
        ...
        try {
            const response = await fetch('{{ route("student.documents.upload") }}', {
                method: 'POST',
                headers: {
