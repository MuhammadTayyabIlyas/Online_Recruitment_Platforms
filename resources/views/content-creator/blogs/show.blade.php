...
<div class="container mx-auto px-4 py-8">
    ...
    @if($blog->status->value === 'rejected' && $blog->admin_feedback)
        <div class="mb-6 bg-red-50 border-l-4 border-red-500 rounded-lg p-6">
            <div class="flex items-start">
                ...
                <div class="flex-1">
                    ...
                    <div class="bg-white rounded p-3 text-red-900 whitespace-pre-line">{{ $blog->admin_feedback }}</div>
                    <div class="mt-4">
                        <a href="{{ route('content-creator.blogs.edit', $blog) }}"
                    ...
                    <div class="space-y-3">
                        @if($blog->meta_title)
                            <div>
                                ...
                                <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded">{{ $blog->meta_title }}</p>
                            </div>
                        @endif
                        ...
                        @if($blog->meta_description)
                            <div>
                                ...
                                <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded">{{ $blog->meta_description }}</p>
                            </div>
                        @endif
                        ...
                        @if($blog->meta_keywords)
                            <div>
                                ...
                                <p class="text-sm text-gray-900 bg-gray-50 p-2 rounded">{{ $blog->meta_keywords }}</p>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="lg:col-span-1 space-y-6">
                ...
                <div class="space-y-3">
                    ...
                    @if($blog->submitted_at)
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-900">{{ $blog->submitted_at->format('M d, Y') }}</span>
                        </div>
                    @endif
                    ...
                    @if($blog->reviewed_at)
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-900">{{ $blog->reviewed_at->format('M d, Y') }}</span>
                        </div>
                    @endif
                    ...
                    @if($blog->published_at)
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm text-gray-900">{{ $blog->published_at->format('M d, Y') }}</span>
                        </div>
                    @endif
            ...
            <div class="bg-gradient-to-br from-blue-50 to-purple-50 rounded-lg shadow-sm border border-blue-200 p-6">
                ...
                <div class="space-y-3">
                    ...
                    @if($blog->canBeSubmitted())
                        <form action="{{ route('content-creator.blogs.submit', $blog) }}" method="POST" onsubmit="return confirm('Submit this blog for admin review?');">
                            @csrf
                            <button type="submit"
                    ...
                    @if($blog->status->canBeWithdrawn())
                        <form action="{{ route('content-creator.blogs.withdraw', $blog) }}" method="POST" onsubmit="return confirm('Withdraw this blog from review?');">
                            @csrf
                            <button type="submit"
                    ...
                    @if($blog->status->value === 'draft')
                        <form action="{{ route('content-creator.blogs.destroy', $blog) }}"
                              method="POST"
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900 font-medium">{{ $blog->category->name }}</span>
                    </div>

                    <div class="flex justify-between">
                        <span class="text-gray-600">Views:</span>
                        <span class="text-gray-900 font-medium">{{ number_format($blog->views_count) }}</span>
                    </div>

                    <div class="flex justify-between">
                        <span class="text-gray-600">Reading Time:</span>
                        <span class="text-gray-900 font-medium">{{ $blog->reading_time }} min</span>
                    </div>

                    <div class="flex justify-between">
                        <span class="text-gray-600">Created:</span>
                        <span class="text-gray-900 font-medium">{{ $blog->created_at->format('M d, Y') }}</span>
                    </div>

                    <div class="flex justify-between">
                        <span class="text-gray-600">Last Updated:</span>
                        <span class="text-gray-900 font-medium">{{ $blog->updated_at->format('M d, Y') }}</span>
                    </div>

                    <div class="flex justify-between">
                        <span class="text-gray-600">Slug:</span>
                        <code class="text-xs bg-gray-100 px-2 py-1 rounded">{{ $blog->slug }}</code>
                    </div>
                </div>
            </div>
        </div>
